...
> **2026-07 更新**：当前实现已简化——移除了邮箱验证码认证（登录仅需邮箱 + 密码，初始化仅需 SETUP_SECRET）和 Redis（限流改为进程内存实现）。本目录中涉及 SMTP/验证码/Redis 的章节为历史设计，已不再适用；部署以根目录 [`DEPLOYMENT.md`](../DEPLOYMENT.md) 为准。
