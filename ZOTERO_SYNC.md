...
当前插件通过 XPI 文件手动升级。更新插件时，在开发机重新执行
`./zotero-plugin/build-xpi.ps1`，然后在 Zotero 的插件管理器中安装新生成的 XPI，
并重启 Zotero。

## Zotero setup

1. 在 Zotero 9 中安装 `zotero-plugin/dist/paper-kb-sync-0.2.1.xpi`。
2. 打开 `Edit -> Settings -> Paper KB Sync`。
3. 填入与 `PUBLIC_BASE_URL` 相同的公开服务器地址。
4. 粘贴网页端 `安全设置` 中只显示一次的令牌，点击“保存令牌并测试连接”。
   成功时页面会明确显示“连接成功，服务器地址和同步令牌已保存”；失败时会显示
   具体失败原因，令牌不会被写入。
